...
static void conf_options_free_internal(ConfOptions *o)
{
    ...
    g_clear_pointer(&o->accel_store, g_object_unref);
}
 ...
 * into the PR_STEREO_* bits of m->mode, preserving the layout/anaglyph bits
 * already present in m->mode. */
static void stereo_mode_pack(struct StereoModeOptions *m)
{
    m->mode = (m->mode & (PR_STEREO_HORIZ | PR_STEREO_VERT | PR_STEREO_FIXED | PR_STEREO_ANAGLYPH | PR_STEREO_HALF)) |
              (m->mirror_right ? PR_STEREO_MIRROR_RIGHT : 0) |
              (m->flip_right   ? PR_STEREO_FLIP_RIGHT : 0) |
              (m->mirror_left  ? PR_STEREO_MIRROR_LEFT : 0) |
              (m->flip_left    ? PR_STEREO_FLIP_LEFT : 0) |
              (m->swap         ? PR_STEREO_SWAP : 0) |
              (m->temp_disable ? PR_STEREO_TEMP_DISABLE : 0);
}
...
#undef COMPARE_OPT
    ...
    stereo_mode_pack(&options->stereo.window);
    stereo_mode_pack(&options->stereo.fs);
...
static void config_window_close_cb(GtkWidget *widget, gpointer data)
{
    ...
    g_clear_pointer(&filter_store, g_object_unref);
...
static void config_tab_windows(GtkWidget *notebook)
{
    ...
    _("<i>%name%</i> results in the filename of the picture.\n"
      "Also available: <i>%collection%</i>, <i>%number%</i>, <i>%total%</i>, <i>%page%</i>, <i>%page_total%</i>,\n"
      "<i>%date%</i>, <i>%size%</i> (filesize), <i>%width%</i>, <i>%height%</i>, <i>%res%</i> (resolution)\n"
...
{
    for (GList *work = c_options->mouse_bindings; work; work = work->next)
    {
        MouseBinding *b = work->data;
...
static void mouse_binding_set_for_action(const gchar *action_name, guint button, GdkModifierType state)
{
    ...
    /* an action can have at most one mouse binding for now, and
     * the same button/state combination cannot be bound to two actions */
    for (GList *next, *work = c_options->mouse_bindings; work; work = next)
    {
        next = work->next; /* need to save this since g_list_delete_link will free the node */
        b = work->data;
        if ((b->button == button && b->state == state) ||
            (b->action_name && strcmp(b->action_name, action_name) == 0))
        {
...
{
    for (GList *work = c_options->mouse_bindings, *next; work; work = next)
    {
        MouseBinding *b = work->data;
        next = work->next;
        if (b->action_name && strcmp(b->action_name, action_name) == 0)
...
static void mouse_binding_store_populate(void)
{
    ...
    items = layout_collect_actions(layout_window_list->data);
...
#define ADD_MIRROR_FLIP_CHECKBOX(col, row, label, mask, field) \
    ...
    ADD_MIRROR_FLIP_CHECKBOX(0, 1, _("Mirror right image"), PR_STEREO_MIRROR_RIGHT, mirror_right);
    ADD_MIRROR_FLIP_CHECKBOX(1, 1, _("Flip right image"),   PR_STEREO_FLIP_RIGHT, flip_right);
...
static void config_tab_stereo(GtkWidget *notebook)
{
    ...
    add_stereo_settings_group(vbox, FALSE);
    add_stereo_settings_group(vbox, TRUE);
}
