...
static void write_global_attributes(GString *outstr, gint indent)
{
    ...
    WRITE_NL(); WRITE_UINT(*options, duplicates_neartime_threshold);
    ...
    WRITE_NL(); WRITE_INT_NAME(*options, stereo.mode, stereo.window.mode);
    WRITE_NL(); WRITE_INT_NAME(*options, stereo.fsmode, stereo.fs.mode);
...
static gboolean load_global_params(const gchar **attribute_names, const gchar **attribute_values)
{
    while (*attribute_names)
    {
        ...
        if (READ_UINT(*options, duplicates_days_threshold)) continue;
        if (READ_UINT(*options, duplicates_neartime_threshold)) continue;
        ...
        if (READ_INT_NAME(*options, stereo.mode, stereo.window.mode)) continue;
        if (READ_INT_NAME(*options, stereo.fsmode, stereo.fs.mode)) continue;
